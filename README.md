...
You need the Rust toolchain installed. Ensure you have a C/C++ compiler (e.g. MinGW/GCC on Windows, GCC on Linux, Clang on macOS) installed and available in your system `PATH` (it is required to compile the static C/C++ OpenH264 bindings).
...
`target/release/silocut.exe` (on Windows) or `target/release/silocut` (on macOS/Linux).
